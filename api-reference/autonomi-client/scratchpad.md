...
{% tab title="Rust" %}
```rust
use autonomi::{Client, SecretKey, Bytes};
...
fn get_funded_wallet() -> Wallet {
    ...
    Wallet::new_from_private_key(network, private_key).expect("Failed to create wallet")
}
...
async fn scratchpad_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Retrieved scratchpad with counter: {}", scratchpad.counter());
    ...
    println!("✓ Updated scratchpad verified with counter: {}", updated.counter());
